...
from preprocessing import patch_slicing
    ...
    def __init__(self, input_dir, output_dir, train=False, retrain=False, retest=False):
        ...
        if not (train or retest):
            ...
            self.model.load_state_dict(
                torch.load(
                    Path("/opt/algorithm/model.training") if execute_in_docker else Path("model.training"),
                    map_location=self.device,
                )
            )
            self.model2.load_state_dict(
                torch.load(
                    Path("/opt/algorithm/model2_mil.training") if execute_in_docker else Path("model2_mil.training"),
                    map_location=self.device,
                )
            )
    ...
    def predict(self, *, input_image: SimpleITK.Image) -> DataFrame:
        ...
        with torch.no_grad():
            for j in range(len(image_data)):
                ...
                patches = patch_slicing(image)              # 64*64 x 3 x 112 x 112

                outputs = []
                outputs2 = []
                checked = np.zeros((64*64, 1))

                for i, _data in enumerate(patches):
                    if _data[0][64][64] == -1:
                        checked[i] = 1
                ...
                for i, _data in enumerate(patches):
                    if checked[i] == 0:
                        if len(_input) == bsize:
                            inp = torch.zeros(bsize, 3, 112, 112)
                            for jj in range(bsize):
                                inp[jj] = _input[jj][1]
                            output = self.model(inp.to(self.device))
                            for jj in range(bsize):
                                outputs.append((_input[jj][0], output[jj].unsqueeze(0)))
                                score = output[jj]
                                if score < 0.01:
                                    checked[_input[jj][0]] = 1
                            _input = []

                        _input.append((i, _data))

                _input2 = []
                for i, _data in enumerate(patches):
                    if checked[i] == 0:
                        if len(_input2) == bsize:
                            inp = torch.zeros(bsize, 3, 112, 112)
                            for jj in range(bsize):
                                inp[jj] = _input2[jj][1]
                            output2 = self.model2(inp.to(self.device))
                            for jj in range(bsize):
                                outputs2.append((_input2[jj][0], output2[jj].unsqueeze(0)))
                            _input2 = []

                        _input2.append((i, _data))

                inp = torch.zeros(len(_input), 3, 112, 112)
                for k in range(len(_input)):
                    inp[k] = _input[k][1]
                output = self.model(inp.to(self.device))
                for k in range(len(_input)):
                    outputs.append((_input[k][0], output[k]))

                inp2 = torch.zeros(len(_input2), 3, 112, 112)
                for k in range(len(_input2)):
                    inp2[k] = _input2[k][1]
                output2 = self.model2(inp2.to(self.device))
                for k in range(len(_input2)):
                    outputs2.append((_input2[k][0], output2[k]))

                np_prediction = postprocessing(outputs, outputs2)
